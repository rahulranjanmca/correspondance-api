...
- Install the heroku cli basis the OS (https://devcenter.heroku.com/articles/heroku-cli#download-and-install)
- Create an app in heroku via Heroku Dashboard
- Install the mlab mongodb addon - (https://elements.heroku.com/addons/mongolab) - and provision this add-on to the above created app
- Once you are navigated to the heroku dashboard, click the `mLab MongoDB` link which should take you to the mlab dashboard which has all the mongo db credentials
- Login to heroku via heroku cli using the command `heroku login`
- Setup the git remote pointing to the heroku app `heroku git:remote -a < your-heroku-app >`
- Set the NPM_PRODUCTION_CONFIG to false `heroku config:set NPM_CONFIG_PRODUCTION=false`
- Set the NODE_ENV to local `heroku config:set NODE_ENV=LOCAL`
- Set the other variables (like above received mlab mongoDB credentials) in the `.env` file
- Remove `*.env` entry from `.gitignore` file - This is needed so that `.env` file is pushed to heroku and the app reads all the environment variables from this environment file
- Add changes to git - `git add .`
- Create a commit message - `git commit -m "< your commit message >"`
- Push to heroku - `git push heroku master`
- You can check the status of the deployment by running `heroku logs --tail`
- After making any changes, you can add, commit and push to Heroku
