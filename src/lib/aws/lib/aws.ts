...
export default class AWS {
  private _s3: Record<string, S3>;
  private _ssm: Record<string, SSM>;
  private _ses: Record<string, SES>;
  private _sts: Record<string, STS>;
  ...
  private _initServices(config: IAWSConfig): void {
    try {
  ...
  createInstance(service: string, awsConfig: IAWSConfig): SSM | S3 | SES | STS | undefined {
    try {
  ...
  public ssm(name: string): SSM {
    return this._ssm[name];
  }

  public ses(name: string): SES {
    return this._ses[name];
  }

  public sts(name: string): STS {
