...
const _sanitizeError = (error: any) => {
  try {
...
const _stringify = (thing: any, native = false) => {
  try {
    let stringified: any;
...
const _isStringEmpty = (str: string) => {
  try {
    if (_.isString(str)) {
...
const _stringToBoolean = (value: any, defaultValueAsBoolean: boolean | any) => {
  try {
