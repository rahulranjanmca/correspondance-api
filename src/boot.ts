...
const environmentVerify = (path: string): void => {
  try {
    dotEnvSafe.config({
      sample: path
    });
...
export const killSelf = async (error: any): Promise<void> => {
  try {
...
export const serverless = (event: any, binaryMimeTypes: any, context: any): Promise<void> => {
...
export const isHealthCheck = (event: any): boolean => {
  try {
