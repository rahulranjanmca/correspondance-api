...
describe('WebResponsesService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw error when entity invalidate', async () => {
      try {
        await service.create(Object.assign({}, templateDetailFields, { type: '' }), 'test');
        assert.fail('should not be here');
      } catch (error) {
        expect(error).toBeInstanceOf(HttpException);
        expect(error.message.message).toBe('type should not be empty, type must be a valid enum value');
      }
