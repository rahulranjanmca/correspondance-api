...
describe('TemplatesService', () => {
  ...
  it('should getLetterCatalog success', async () => {
    ...
    expect(
      (await service.getLetterCatalog({ marketSegment: 'test', subject: 'test', audience: 'test', userId: 'test', role: 'test' })).length
    ).toBe(1);
    expect(
      (await service.getLetterCatalog({ marketSegment: 'missing', subject: 'test', audience: 'test', userId: 'test', role: 'test' })).length
    ).toBe(0);
  });
  ...
  it('should getFormCatalog success', async () => {
    ...
    expect((await service.getFormCatalog({ marketSegment: 'test', state: 'test', userId: 'test', role: 'test' })).length).toBe(1);
    expect((await service.getFormCatalog({ marketSegment: 'missing', state: 'test', userId: 'test', role: 'test' })).length).toBe(0);
  });
