...
describe('TemplatesService', () => {
  ...
  it('should getLetterCatalog success', async () => {
    mockFunction.mockResolvedValue([
      ...
      { segment: 'test', subject: 'test', audience: 'test', DocumentType: 'form' }
    ]);
    expect(
  ...
  it('should getLetterCatalog with no response dto success', async () => {
    mockFunction.mockResolvedValue([
      { margetSegment: 'test1', subject: 'test', audience: 'test', DocumentType: 'letter' },
      { margetSegment: 'test1', subject: 'test', audience: 'test', DocumentType: 'form' }
    ]);
    expect(
      await service.getLetterCatalog({ marketSegment: 'test', subject: 'test', audience: 'test', userId: 'test', role: 'test' })
    ).toBeDefined();
  ...
  it('should getFormCatalog success', async () => {
    mockFunction.mockResolvedValue([
      ...
      { segment: 'test', state: 'test', DocumentType: 'form' }
    ]);

    expect(await service.getFormCatalog({ marketSegment: 'test', state: 'test', userId: 'test', role: 'test' })).toBeDefined();
  });

  it('should getFormCatalog success with no template dtto', async () => {
    mockFunction.mockResolvedValue([
      { margetSegment: 'test1', state: 'test', DocumentType: 'letter' },
      { margetSegment: 'test1', state: 'test', DocumentType: 'form' }
    ]);
