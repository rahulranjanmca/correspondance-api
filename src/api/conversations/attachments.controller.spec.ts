...
describe('Attachments Controller', () => {
  ...
  it('should download fail', async () => {
    ...
    mockFunction.mockRejectedValue(buf);
    const mockResponse = ({ set: () => mockResponse, send: () => {} } as unknown) as Response;
    try {
      await controller.download('test', 'test', ({} as unknown) as UserAndRoleCommonQuery, mockResponse);
    } catch (error) {
      expect(error).toBeDefined();
    }
  });
  ...
  it('should download success', async () => {
    const buf = Buffer.from('test', 'utf-8');
    mockFunction.mockResolvedValue(buf);
    const mockResponse = ({
      set: () => mockResponse,
      send: () => {},
      on: () => {},
    ...
    expect(await controller.download('test', 'test', ({} as unknown) as UserAndRoleCommonQuery, mockResponse)).toBeUndefined();
  });
});
