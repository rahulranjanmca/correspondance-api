...
describe('Attachments Controller', () => {
  ...
  it('should downloadAttachment success', async () => {
    ...
    await controller.download('id', 'attachmentId', ({} as unknown) as UserAndRoleCommonQuery, response);
    expect(response.set).toBeCalledTimes(1);

    mockFunction.mockRejectedValue('test');
    try {
      await controller.download('id', 'attachmentId', ({} as unknown) as UserAndRoleCommonQuery, ({} as unknown) as Response);
    } catch (error) {
      expect(error).toBeDefined();
    }
  });
