...
describe('ConversationsService', () => {
  ...
  beforeEach(async () => {
    ...
    const catalog1 = Object.assign(new CatalogEntity(), testCatalogData[0]);
    const catalog2 = Object.assign(new CatalogEntity(), testCatalogData[1]);

    mockGetCatalogList.mockResolvedValue([catalog1, catalog2]);
  ...
  describe('create conversation', () => {
    ...
    it('should create conversation success with no approvalChain', async () => {
      const ret = await service.createConversation(
        { userId: 'test_user', role: 'test_role' },
        { caseId: 'test_caseId', formId: '', formQuantity: [] }
      );

      expect(ret.caseId).toBe('test_caseId');
      const db = await conversationRepository.find();
      expect(db.length).toBe(1);
      expect(db[0].approvalChain).toBe(undefined);
    });
  ...
  describe('update conversation', () => {
    ...
    it('should update conversastion success without formQuantity', async () => {
      const initConversation = await saveAndGetConversation();
      const newConversation = new ConversationDto(initConversation);
      ...
      expect(ret.formQuantity).toBe(undefined);
      const dbConversation = await conversationRepository.find();
      expect(dbConversation.length).toBe(1);
      expect(dbConversation[0].formQuantity).toBe(undefined);
    });
  });
  ...
  describe('approve current step', () => {
    ...
    it('should throw error if repository.save did not happen correctly', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
      ...
      const originalSaveFn = conversationRepository.save;
      conversationRepository.save = jest.fn().mockRejectedValue('error');
      ...
      conversationRepository.save = originalSaveFn;
    });
  ...
  describe('addAttachment', () => {
    it('should addAttachment success', async () => {
      ...
      const dbConversation = await saveAndGetConversation(conversation);
  ...
  describe('reject', () => {
    ...
    it('should successfully reject conversation attachments', async () => {
      mockDownloadFromS3.mockResolvedValue('s3Key');
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
  ...
  describe('downloadAttachment', () => {
    it('should check downloadAttachment success', async () => {
      mockDownloadFromS3.mockResolvedValue('s3Key');
      const conversation = new Conversation();
      conversation.attachments = [
        {
    ...
    it('should throw error if attachments not present', async () => {
      const dbConversation = await saveAndGetConversation();
      ...
      try {
        ...
        expect(err.getResponse()).toEqual({ message: 'Requested entity is not found in database' });
        expect(err.getStatus()).toEqual(404);
      }
    });

    it('should throw error if requested attachment is not part of conversation attachments', async () => {
      mockDownloadFromS3.mockResolvedValue('s3Key');
      const conversation = new Conversation();
      conversation.attachments = [
        {
  ...
  it('should be able to instantiate form.data.entity', () => {
    const data = {
      key1: 'val1',
      key2: 'val2'
    };
    const formDataEntity = new FormData();
    formDataEntity.formId = 'form1';
    formDataEntity.data = data;

    const formDataDto = new FormDataDto();
    formDataDto.formId = 'form1';
    formDataDto.data = data;

    expect(formDataDto.formId).toEqual('form1');
    expect(formDataDto.data).toEqual(data);
    expect(formDataEntity.formId).toEqual('form1');
    expect(formDataEntity.data).toEqual(data);
  });
});
