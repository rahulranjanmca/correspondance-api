...
describe('ConversationsService', () => {
  ...
  class MockCommonService {
    ...
    async downloadFromS3(attachmentId: string): Promise<Buffer> {
      return mockDownloadFromS3(attachmentId);
    }
  }
  ...
  beforeEach(async () => {
    ...
    mockGetCatalogList.mockResolvedValue(plainToClass(CatalogEntity, testCatalogData));
  ...
  describe('create conversation', () => {
    ...
    it('should create conversation success without formId', async () => {
      const conversationCreateRequest = ({ caseId: 'test_caseId', formQuantity: [] } as unknown) as ConversationCreateRequest;
      const ret = await service.createConversation({ userId: 'test_user', role: 'build_user' }, conversationCreateRequest);

      expect(ret.caseId).toBe('test_caseId');
      const db = await conversationRepository.find();
      expect(ret.approvalChain).toBeUndefined();
      expect(db.length).toBe(1);
    });
  ...
  describe('update conversation', () => {
    ...
    it('should update conversastion success without formQuantity', async () => {
      ...
      const dbConversation = await conversationRepository.find();
      expect(dbConversation.length).toBe(1);
      expect(dbConversation[0].formQuantity.length).toEqual(0);
    });
  });
  ...
  describe('approve conversation', () => {
    it('should approve success', async () => {
      ...
      try {
        plainToClass(ConversationDto, { formQuantity: [], attachments: [], approvalChain: [] });
        plainToClass(ConversationPreviewRequestDto, { instance: {} });
        plainToClass(ConversationUpdateBodyDto, dbConversation);
        plainToClass(ConversationCreateRequest, { formQuantity: [] });
      } catch (error) {}
  ...
  describe('approve current step', () => {
    ...
    it('should not approve current step success for none step', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: []
    ...
    it('should not approve current step success for save exception', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
      ...
      mockSendDocumentApprovalOrThrow.mockResolvedValue(null);
      jest.spyOn(conversationRepository, 'save').mockRejectedValueOnce(Error('test'));
      try {
        await service.approveCurrentStep(dbConversation._id.toString(), {
          userId: dbConversation.userId,
          role: dbConversation.role
  ...
  describe('downloadAttachment', () => {
    it('should downloadAttachment success', async () => {
      const dbConversation = await saveAndGetConversation(({ attachments: [{ id: 'test' }] } as unknown) as Conversation);
      mockDownloadFromS3.mockResolvedValue('buffer');
      expect(
      ...
      expect(mockDownloadFromS3).toBeCalledTimes(1);
      expect(mockDownloadFromS3).toBeCalledWith('test');
    });
    it('should not downloadAttachment success for no attachment', async () => {
      const dbConversation = await saveAndGetConversation();
      ...
      expect(mockDownloadFromS3).not.toBeCalled();
    });

    it('should not downloadAttachment success for not match', async () => {
      const dbConversation = await saveAndGetConversation(({ attachments: [{ id: 'test' }] } as unknown) as Conversation);
  ...
  describe('reject', () => {
    ...
    it('should reject success with clear s3 attachments', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        attachments: [{ id: 'test' }],
      ...
      expect(mockDeleteFromS3).toBeCalledTimes(1);
      expect(mockDeleteFromS3).toBeCalledWith('test');
    });
