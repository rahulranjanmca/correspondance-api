...
describe('ConversationsService', () => {
  ...
  describe('create conversation', () => {
    ...
    it('should create conversation success for form with Approval_template', async () => {
      const ret = await service.createConversation(
        { userId: 'test_user', role: 'build_user' },
  ...
  describe('update conversation', () => {
    it('should update conversation success', async () => {
    ...
    it('should update conversation success for letter conversation', async () => {
      const initConversation = await saveAndGetConversation();
      const newConversation = new ConversationDto(initConversation);
      newConversation.formQuantity = [];
  ...
  describe('approve conversation', () => {
    it('should approve success', async () => {
      ...
      try {
        plainToClass(ConversationDto, {
          formQuantity: [{ formId: 'formId', quantity: 1 }],
          attachments: [{ id: 'id' }],
          approvalChain: [{ role: 'role' }]
        });
        plainToClass(ConversationPreviewRequestDto, { instance: [{ id: 'id', contentType: 'contentType' }], inputData: {} });
        plainToClass(ConversationUpdateBodyDto, { formQuantity: [{ formId: 'formId', quantity: 1 }] });
        plainToClass(ConversationCreateRequest, { caseId: 'caseId', formId: 'formId', formQuantity: [{ formId: 'formId', quantity: 1 }] });
      } catch (error) {}
  ...
  describe('approve current step', () => {
    ...
    it('should not approve current step success for send exception', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
    ...
    it('should not approve current step success for save exception', async () => {
      ...
      mockSendDocumentApprovalOrThrow.mockResolvedValue(null);
      jest.spyOn(conversationRepository, 'save').mockRejectedValue('test');
      try {
        await service.approveCurrentStep(dbConversation._id.toString(), {
          userId: dbConversation.userId,
          role: dbConversation.role
  ...
  describe('addAttachment', () => {
    ...
    it('should not addAttachment success for get file type failed', async () => {
      const dbConversation = await saveAndGetConversation(({ attachments: [] } as unknown) as Conversation);
  ...
  describe('downloadAttachment', () => {
    it('should download Attachment success', async () => {
      const dbConversation = await saveAndGetConversation(({ attachments: [{ id: 'test' }] } as unknown) as Conversation);
      ...
      ).toStrictEqual(Buffer.from('test'));
    });

    it('should download Attachment success for no attachment', async () => {
      const dbConversation = await saveAndGetConversation();
    ...
    it('should not download Attachment success for not match', async () => {
      const dbConversation = await saveAndGetConversation(({ attachments: [{ id: 'test' }] } as unknown) as Conversation);
  ...
  describe('reject', () => {
    ...
    it('should reject success for no attachment', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
