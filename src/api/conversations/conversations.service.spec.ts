...
describe('ConversationsService', () => {
  ...
  describe('create conversation', () => {
    ...
    it('should not set approval chain for empty form id', async () => {
      const ret = await service.createConversation(
        { userId: 'test_user', role: 'build_user' },
  ...
  describe('update conversation', () => {
    ...
    it('should skip uploadToS3 for empty form quantity', async () => {
      const initConversation = await saveAndGetConversation();
      const newConversation = new ConversationDto(initConversation);
      newConversation.formQuantity = [];

      await service.updateConversation(initConversation._id.toString(), ({} as unknown) as UserAndRoleCommonQuery, newConversation);

      expect(mockUploadToS3).toBeCalledTimes(0);
    });
  });
  ...
  describe('approve conversation', () => {
    it('should approve success', async () => {
      const dbConversation = await saveAndGetConversation(({
        ...
        attachments: [
          {
            id: '417ebad5-c2cc-4739-a054-4249c513b904',
            originalFilename: 'abc',
            contentType: 'abc',
            attachmentLocation: 'abc'
          }
        ],
        approvalChain: [
          {
  ...
  describe('approve current step', () => {
    ...
    it('should throw error for conversation repository save failure', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
      ...
      jest.spyOn(conversationRepository, 'save').mockReset();
    });
  ...
  describe('addAttachment', () => {
    ...
    it('should addAttachment success with non empty attachments', async () => {
      const dbConversation = await saveAndGetConversation();
      ...
      // add attachment again to verify attachment can be added to the previous list
      const ret = await service.addAttachment(
        dbConversation._id.toString(),
        { userId: dbConversation.userId, role: dbConversation.role },
        { contentType: 'application/pdf', originalFilename: 'test.pdf' },
  ...
  describe('downloadAttachment', () => {
    it('should downloadAttachment success', async () => {
      const dbConversation = await saveAndGetConversation();
      (fromStream as jest.Mock).mockResolvedValue({ mime: 'application/pdf' });
      const attachmentDto = await service.addAttachment(
        dbConversation._id.toString(),
        { userId: dbConversation.userId, role: dbConversation.role },
        { contentType: 'application/pdf', originalFilename: 'test.pdf' },
        Buffer.from('test')
      ...
      const ret = await service.downloadAttachment(
        dbConversation._id.toString(),
        { userId: dbConversation.userId, role: dbConversation.role },
        attachmentDto.id
      );
    ...
    it('should not downloadAttachment success for attachments empty', async () => {
      const dbConversation = await saveAndGetConversation();
      try {
    ...
    it('should not downloadAttachment success for attachment id mismatch', async () => {
      const dbConversation = await saveAndGetConversation();
      (fromStream as jest.Mock).mockResolvedValue({ mime: 'application/pdf' });
      const attachmentDto = await service.addAttachment(
        dbConversation._id.toString(),
      ...
      try {
        await service.downloadAttachment(
          dbConversation._id.toString(),
          { userId: dbConversation.userId, role: dbConversation.role },
          attachmentDto.id + 'invalid_postfix'
        );
        assert.fail('should not be here');
      } catch (error) {
        expect(error.message.message).toBe('Requested entity is not found in database');
  ...
  describe('reject', () => {
    it('should reject success with non empty attachments', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
        ...
        attachments: [{ id: 'attachment-id-1' }]
      } as unknown) as Conversation);
    ...
    it('should reject success with empty attachments', async () => {
      const dbConversation = await saveAndGetConversation(({
        role: 'role step 1',
        status: DocumentStatus.draft,
        approvalChain: [
