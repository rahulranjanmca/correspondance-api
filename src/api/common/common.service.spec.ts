...
describe('CommonService', () => {
  ...
  describe('buildDocument', () => {
    ...
    it('should build document success when formQuantity is specified in params', async () => {
      mockGetFunction.mockImplementation(() => {
        return from([
          {
            status: 200,
  ...
  describe('downloadFromS3', () => {
    it('should downloadFromS3 success', async () => {
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = 'S3';

      service = new CommonService(
        ({ post: mockPostFunction, get: mockGetFunction, put: mockPutFunction } as unknown) as HttpService,
        auditRecordEntityRepository,
        ({} as unknown) as JwtService
      );

      const response = await service.downloadFromS3('s3Key');
      expect(response).toEqual(new Buffer('testString'));
    });
  });

  describe('uploadToS3', () => {
    it('should uploadToS3 success', async () => {
      let response = await service.uploadToS3(new Buffer('testString'), 's3Key');
      expect(response).toEqual('temp/s3Key');
      ...
      response = await service.uploadToS3(new Buffer('testString'), 's3Key');
      expect(response).toEqual('testLocation');
    });
  });

  describe('deleteFromS3', () => {
    it('should deleteFromS3 success', async () => {
      let response = await service.deleteFromS3('s3Key');
      ...
      response = await service.deleteFromS3('s3Key');
      expect(response).toBe(undefined);

      try {
        await service.deleteFromS3('error');
      } catch (err) {
        expect(err.getResponse()).toEqual({ message: 'delete From S3 failed' });
        expect(err.getStatus()).toEqual(500);
      }
    });
  });

  it('should create temp path if it does not exist', () => {
