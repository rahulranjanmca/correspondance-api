...
import { existsSync, promises as pfs, rmdirSync } from 'fs';
...
describe('CommonService', () => {
  ...
  beforeAll(async () => {
    if (existsSync(process.env.WM_CC_TEMPORARY_PATH || '')) {
      rmdirSync(process.env.WM_CC_TEMPORARY_PATH || '');
    }
  });
  ...
  describe('buildDocument', () => {
    it('should build document success', async () => {
      mockGetFunction.mockImplementation(() => {
        return from([
          {
            status: 200,
    ...
    it('should not build document success for no form id', async () => {
      mockGetFunction.mockImplementation(() => {
        return from([
          {
            status: 200,
  ...
  describe('downloadFromS3', () => {
    it('should download from s3 success', async () => {
      const objectStoreType = process.env.WM_CCC_OBJECT_STORAGE_TYPE;
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = 'S3';
      mockS3GetObject.mockImplementation(() => ({
        promise() {
          return Promise.resolve({ Body: Buffer.from('test document') });
        }
      }));
      const commonService = new CommonService(({} as unknown) as HttpService, auditRecordEntityRepository, ({} as unknown) as JwtService);
      const ret = await commonService.downloadFromS3('test');
      expect(ret.length).toBe(13);
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = objectStoreType;
    });
  });
  describe('uploadToS3', () => {
    it('should upload to s3 success', async () => {
      const objectStoreType = process.env.WM_CCC_OBJECT_STORAGE_TYPE;
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = 'S3';
      mockS3UploadObject.mockImplementation(() => ({
        promise() {
          return Promise.resolve({ Location: 'test/file' });
        }
      }));
      const commonService = new CommonService(({} as unknown) as HttpService, auditRecordEntityRepository, ({} as unknown) as JwtService);
      const ret = await commonService.uploadToS3(Buffer.from('test'), 'test');
      expect(ret).toBe('test/file');
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = objectStoreType;
    });
    it('should write to local fs success', async () => {
      const mockWriteFile = jest.fn();
      jest.spyOn(pfs, 'writeFile').mockImplementationOnce(mockWriteFile);
      mockWriteFile.mockResolvedValue('');
      await service.uploadToS3(Buffer.from('test'), 'test');
  ...
  describe('deleteFromS3', () => {
    it('should delete from s3 success', async () => {
      ...
      mockS3DeleteObject.mockImplementation(() => ({
        promise: mockDelete
      }));
      mockDelete.mockResolvedValue('');
      const commonService = new CommonService(({} as unknown) as HttpService, auditRecordEntityRepository, ({} as unknown) as JwtService);
      await commonService.deleteFromS3('test');
      expect(mockDelete).toBeCalledTimes(1);
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = objectStoreType;
    });

    it('should delete from s3 failed when get error', async () => {
      const objectStoreType = process.env.WM_CCC_OBJECT_STORAGE_TYPE;
      process.env.WM_CCC_OBJECT_STORAGE_TYPE = 'S3';
      const mockDelete = jest.fn();
      mockS3DeleteObject.mockImplementation(() => ({
      ...
      mockDelete.mockRejectedValue(Error('delete error'));
      const commonService = new CommonService(({} as unknown) as HttpService, auditRecordEntityRepository, ({} as unknown) as JwtService);
      try {
        await commonService.deleteFromS3('test');
        assert.fail('should not be here');
    ...
    it('should write to local fs success', async () => {
      ...
      jest.spyOn(pfs, 'unlink').mockImplementationOnce(mockUnlink);
      mockUnlink.mockResolvedValue('');
      await service.deleteFromS3('test');
      expect(mockUnlink).toBeCalledTimes(1);
    });
  });
});
