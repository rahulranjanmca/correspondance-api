...
describe('CommonService', () => {
  ...
  describe('buildDocument', () => {
    ...
    it('should build document success with attachments when formQuantity is passed', async () => {
      mockGetFunction.mockImplementation(() => {
        return from([
          {
            status: 200,
      ...
      const ret = await service.buildDocument(
        ({
          formQuantity: [{ formId: 'test_form', quantity: 1 }],
          userId: 'test_userId',
          attachments: [
            {
              contentType: 'application/pdf',
  ...
  it('should uploadToS3', async () => {
    expect(await service.uploadToS3(Buffer.from('test'), 'test-upload')).toEqual('temp/test-upload');
  });

  it('should deleteFromS3', async () => {
    expect(await service.deleteFromS3('test-upload')).toBeUndefined();
  });
});

describe('CommonServiceS3', () => {
  let service: CommonService;
  const mongod = new MongoMemoryServer();
  const mockPostFunction = jest.fn();
  const mockPutFunction = jest.fn();
  ...
  it('should uploadToS3', async () => {
    // @ts-ignore
    mockS3Upload.mockImplementation(params => {
      return {
        promise() {
    ...
    expect(await service.uploadToS3(Buffer.from('test'), 'test-upload')).toEqual('test');
  });

  it('should deleteFromS3', async () => {
    // @ts-ignore
    mockS3Delete.mockImplementation(params => {
      return {
        promise() {
    ...
    expect(await service.deleteFromS3('test-delete')).toBeUndefined();
  });

  it('should deleteFromS3 fail with error', async () => {
    // @ts-ignore
    mockS3Delete.mockImplementation(params => {
      return {
        promise() {
    ...
    try {
      await service.deleteFromS3('test-delete');
    } catch (e) {
      expect(e.message.message).toEqual('delete From S3 failed');
    }
  });

  it('should downloadS3', async () => {
    // @ts-ignore
    ...
    expect(await service.downloadFromS3('test-download')).toBeDefined();
  });
});
